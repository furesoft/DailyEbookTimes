...
public partial class YogaNode
{
    ...
    public void CalculateLayout(double? ownerWidth, double? ownerHeight, YogaDirection ownerDirection)
    {
        ...
        YogaConfig config)
    {
        ...
        {
            if (YogaMath.doublesEqual(layout.CachedLayout.AvailableWidth, availableWidth) &&
                YogaMath.doublesEqual(layout.CachedLayout.AvailableHeight, availableHeight) &&
        ...
        else
        {
            for (var i = 0; i < layout.NextCachedMeasurementsIndex; i++)
            {
                if (YogaMath.doublesEqual(layout.CachedMeasurements[i].AvailableWidth, availableWidth) &&
                    YogaMath.doublesEqual(layout.CachedMeasurements[i].AvailableHeight, availableHeight) &&
        ...
        YogaConfig config)
    {
        ...
        var totalOuterFlexBasis = (double?)0F;
        ...
        var totalLineCrossDim = (double?)0F;

        // Max main dimension of all the lines.
        var maxLineMainDim = 0d;
        ...
        {
            var crossDimLead = (double?)0F;
            var currentLead = (double?)leadingPaddingAndBorderCross;
            ...
            for (var i = 0; i < lineCount; i++)
            {
                ...
                var lineHeight = (double?)0d;
                var maxAscentForCurrentLine = 0d;
                var maxDescentForCurrentLine = 0d;
                            ...
                            switch (GetAlign(child))
                            {
                                ...
                                case YogaAlign.Stretch:
                                {
                                    ...
                                    if (!child.IsStyleDimensionDefined(crossAxis, availableInnerCrossDim))
                                    {
                                        ...
                                        if (!(YogaMath.doublesEqual(childWidth, child.Layout.MeasuredDimensions[YogaDimension.Width])
                                              && YogaMath.doublesEqual(childHeight, child.Layout.MeasuredDimensions[YogaDimension.Height])))
                                        {
                                            child.LayoutNode(
                                                childWidth,
                                                childHeight,
    ...
    private double? CalculateAvailableInnerDim(
        ...
        double? ownerDim)
    {
        ...
        if (availableInnerDim != null)
        {
            ...
            var maxInnerDim = maxInnerResolved == null ? double.MaxValue : maxInnerResolved.Value - paddingAndBorder;
    ...
    private void ComputeFlexBasisForChildren(
        ...
        YogaConfig config,
        bool performLayout,
        ref double? totalOuterFlexBasis)
    {
            ...
            foreach (var child in children)
            {
                if (child.IsNodeFlexible())
                {
                    if (singleFlexChild != null ||
                        YogaMath.doublesEqual(child.ResolveFlexGrow(), 0.0f) ||
                        YogaMath.doublesEqual(child.ResolveFlexShrink(), 0.0f))
                    {
    ...
    private double? BoundAxisWithinMinAndMax(
        YogaFlexDirection axis,
        double? value,
        double? axisSize)
    {
        ...
        YogaConfig config)
    {
        ...
        var childWidth = default(double?);
        var childHeight = default(double?);
        ...
        else
        {
            ...
            // flex basis).
            childWidth = default(double?);
            childHeight = default(double?);
    ...
    private static double? CalculateBaseline(YogaNode node)
    {
        var baseline = default(double?);
        ...
        int lineCount)
    {
        ...
        var sizeConsumedOnCurrentLineIncludingMinConstraint = (double?)0F;
    ...
    private static void DistributeFreeSpaceFirstPass(
        ...
        double? availableInnerWidth)
    {
        double? flexShrinkScaledFactor;
        double flexGrowFactor;
        double? baseMainSize;
        double? boundMainSize;
        var deltaFreeSpace = (double?)0d;
    ...
    private double? DistributeFreeSpaceSecondPass(
        ref YogaCollectFlexItemsRowValues collectedFlexItemsValues,
        YogaFlexDirection mainAxis,
        YogaFlexDirection crossAxis,
        double? mainAxisOwnerSize,
        ...
        YogaConfig config)
    {
        var childFlexBasis = (double?)0d;
        var flexShrinkScaledFactor = (double?)0d;
        var flexGrowFactor = 0d;
        var deltaFreeSpace = (double?)0F;
        var isMainAxisRow = mainAxis.IsRow();
        var isNodeFlexWrap = Style.FlexWrap != YogaWrap.NoWrap;
        ...
        foreach (var currentRelativeChild in collectedFlexItemsValues.RelativeChildren)
        {
            ...
            var childCrossSize = default(double?);
    ...
    private void JustifyMainAxis(
        ...
        ref double? mainAxisOwnerSize,
        ref double? ownerWidth,
        ref double? availableInnerMainDim,
        ref double? availableInnerCrossDim,
        ref double? availableInnerWidth,
        ref bool performLayout)
    {
        ...
        var leadingMainDim = (double?)0F;
        var betweenMainDim = (double?)0F;
    ...
    private double? GetDimensionWithMargin(YogaFlexDirection axis, double? widthSize)
    {
        return Layout.MeasuredDimensions[Dimension[axis]] + GetLeadingMargin(axis, widthSize) + GetTrailingMargin(axis, widthSize);
    }

    private bool IsStyleDimensionDefined(YogaFlexDirection axis, double? ownerSize)
    {
    ...
    private double? GetPaddingAndBorderForAxis(YogaFlexDirection axis, double? widthSize)
    {
    ...
    private void RoundToPixelGrid(double pointScaleFactor, double? absoluteLeft, double? absoluteTop)
    {
        ...
        var hasFractionalWidth = !YogaMath.doublesEqual(((nodeWidth * pointScaleFactor) % 1.0F), 0) && !YogaMath.doublesEqual(((nodeWidth * pointScaleFactor) % 1.0F), 1.0F);
        var hasFractionalHeight = !YogaMath.doublesEqual(((nodeHeight * pointScaleFactor) % 1.0F), 0) && !YogaMath.doublesEqual(((nodeHeight * pointScaleFactor) % 1.0F), 1.0F);
    ...
    private static bool MeasureModeSizeIsExactAndMatchesOldMeasuredSize(YogaMeasureMode sizeMode, double? size, double? lastComputedSize)
    {
        return sizeMode == YogaMeasureMode.Exactly && YogaMath.doublesEqual(size, lastComputedSize);
    }

    private static bool MeasureModeOldSizeIsUnspecifiedAndStillFits(YogaMeasureMode sizeMode, double? size, YogaMeasureMode lastSizeMode, double? lastComputedSize)
    {
        return sizeMode == YogaMeasureMode.AtMost
               && lastSizeMode == YogaMeasureMode.Undefined
               && ((size > lastComputedSize || size == lastComputedSize) || YogaMath.doublesEqual(size, lastComputedSize));
    }

    private static bool MeasureModeNewMeasureSizeIsStricterAndStillValid(YogaMeasureMode sizeMode, double? size, YogaMeasureMode lastSizeMode, double? lastSize, double? lastComputedSize)
    {
        return lastSizeMode == YogaMeasureMode.AtMost
               ...
               && lastSize > size
               && (lastComputedSize < size || lastComputedSize == size || YogaMath.doublesEqual(size, lastComputedSize));
    }
        ...
        YogaConfig config)
    {
        ...
        var hasSameWidthSpec = lastWidthMode == widthMode && YogaMath.doublesEqual(effectiveLastWidth, effectiveWidth);
        var hasSameHeightSpec = lastHeightMode == heightMode && YogaMath.doublesEqual(effectiveLastHeight, effectiveHeight);
